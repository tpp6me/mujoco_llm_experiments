...
def run_visual_episode(
    ...
    if hasattr(session, 'max_calls'):
        sess_max_calls = _validate_budget(getattr(session, 'max_calls'), deadline)[0]
    ...
    if hasattr(session, 'deadline'):
        ...
        sess_dl_val = _validate_budget(max_calls, sess_deadline)[1]
    ...
    try:
        while True:
            ...
            try:
                ...
                if action_response.get('status') not in ('completed', 'rejected', 'failed'):
                    raise ValueError('Action response has missing or unknown status')
                # VisualSession's stale-ID rejection has no execution timestamps.
                # Completed/failed executions must supply valid timing before counting.
                end_time = None
                if action_response['status'] != 'rejected' or any(
                    key in action_response for key in ('start_time_s', 'end_time_s')
                ):
                    start_time = _validate_finite_float(action_response.get('start_time_s'), 'start_time_s')
                    end_time = _validate_finite_float(action_response.get('end_time_s'), 'end_time_s')
                    if start_time < sim_time - 1e-6 or end_time < start_time - 1e-6:
                        raise ValueError('Action response timestamps regress')
        ...
        # A failed log write cannot be represented as a successfully archived run.
        termination_reason = 'filesystem_error'
        terminal_error = 'Episode artifact write failed'
        errors += 1
        raise
    except Exception as exc:
        ...
        if not report_written:
            ...
            if hasattr(session, 'env') and session.env is not None and hasattr(session.env, 'data'):
                try:
                    sim_time_final = _validate_finite_float(session.env.data.time, 'final simulation time')
                except (ValueError, TypeError, AttributeError) as exc:
                    evaluator_error = f'{evaluator_error or ""} Final simulation time unavailable: {exc}'.strip()
            ...
            report = {
                ...
                'execution_outcomes_unknown': execution_outcomes_unknown,
                'refusals': refusals,
                'errors': errors,
