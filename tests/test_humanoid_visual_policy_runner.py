...
class VisualPolicyRunnerTests(unittest.TestCase):
    ...
    def test_invalid_result_timing_never_counts_completion(self):
        env = Environment()
        env.reset(820, randomize=True)
        observation = VisualSession(env, MockRenderer()).capture()
        for end in (None, 'broken', float('nan'), True, 0.4):
            with self.subTest(end=end), tempfile.TemporaryDirectory() as tmp:
                session = SimpleNamespace(
                    capture=lambda: copy.deepcopy(observation),
                    execute=lambda *_: {'status': 'completed', 'start_time_s': 0.5,
                                        'end_time_s': end})
                folder = Path(tmp) / 'run'
                report = run_visual_episode(folder, session, HoldStub(0.1))
                ...
                self.assertEqual(report['execution_outcomes_unknown'], 1)
                self.assertIsNone(report['simulated_time_s'])
                record = json.loads((folder / 'call_001.json').read_text())
                self.assertEqual(record['status'], 'malformed_action_result')

    def test_execution_exception_retains_unknown_movement_and_archive_error(self):
        env = Environment()
        env.reset(820, randomize=True)
        observation = VisualSession(env, MockRenderer()).capture()
        def broken_execute(*_):
            raise RuntimeError('could have moved before failing')
        def broken_archive(*_):
            raise OSError('archive unavailable')
        session = SimpleNamespace(capture=lambda: copy.deepcopy(observation),
                                  execute=broken_execute,
                                  env=SimpleNamespace(save=broken_archive))
        with tempfile.TemporaryDirectory() as tmp:
            report = run_visual_episode(Path(tmp) / 'run', session, HoldStub(0.1))
            self.assertEqual(report['termination_reason'], 'execution_exception')
            self.assertEqual(report['execution_outcomes_unknown'], 1)
            self.assertIsNone(report['simulated_time_s'])
            self.assertIn('archive unavailable', report['evaluator_error'])
