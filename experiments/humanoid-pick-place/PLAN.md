...
- [x] AGY implements and tests C1/C2 prompt selection and robot-only geometry bounds under [task 007](../../coordination/agy/tasks/007-c2-implementation.md); offline qualification and completion report submitted for review. C2 remains unfrozen and unexecuted until Codex review and explicit authorization.
- [ ] Codex reviews Task 007, freezes the C2 protocol, and decides whether to issue an execution task for the declared C2 episode.
- [ ] After review and protocol freeze, issue a separate AGY execution/evidence task for the one declared C2 episode. AGY operates the frozen harness; signed-in Codex makes every VLA decision; Codex reviews the outcome and plans the successor.
